...
@section('content')
@php
    // PENGAMAN 1: Gunakan \Illuminate\Support\Str agar sistem tidak bingung
    // PENGAMAN 2: Gunakan ($event->category->name ?? '') agar tidak error jika kategori kosong
    $categoryName = strtolower($event->category->name ?? '');
    $posterUrl = asset('assets/concert.png'); // Gambar Default
    
    if (\Illuminate\Support\Str::contains($categoryName, 'workshop')) {
        $posterUrl = asset('assets/workshop.png');
    } elseif (\Illuminate\Support\Str::contains($categoryName, 'it') || \Illuminate\Support\Str::contains($categoryName, 'code')) {
        ...
        <div class="lg:col-span-1">
            <div class="sticky top-32">
                <img src="{{ $posterUrl }}" alt="{{ $event->title }}"
                    class="w-full rounded-[2.5rem] shadow-2xl border-8 border-white object-cover aspect-video">
            ...
            <div
                ...
                <div class="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                    ...
                    <div>
                        ...
                        @else
                            <button disabled class="inline-block px-10 py-5 bg-slate-300 text-slate-500 rounded-2xl font-black text-xl cursor-not-allowed">
                                Tiket Habis
                            </button>
                        @endif
                    </div>
